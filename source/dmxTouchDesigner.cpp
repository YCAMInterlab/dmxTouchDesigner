...
CPlusPlusCHOPExample::CPlusPlusCHOPExample(const CHOP_NodeInfo *info) : myNodeInfo(info)
{
	myExecuteCount = 0;
}
...
CPlusPlusCHOPExample::getOutputInfo(CHOP_OutputInfo *info)
{
	...
	if (info->inputArrays->numCHOPInputs > 3)
	{
		return true;
	}
	else
...
const char*
CPlusPlusCHOPExample::getChannelName(int index, void* reserved)
{
	return "Chan5";
}
								...
								void* reserved)
{
	...
	if (inputs->numCHOPInputs > 0)
	{
		...
		int i=0;
		int device_num = inputs->floatInputs[0].values[0];
		int state = inputs->floatInputs[0].values[1];
		BOOL res = 0;
		Num_Devices = FTDI_ListDevices();

		// we'll open the first one only
		int dev = inputs->floatInputs[0].values[1];
		...
		int numchans = inputs->floatInputs[1].values[0];
		int values[512];
		int channels[512];

		for(int i = 0; i < numchans; i++){
				channels[i] = inputs->CHOPInputs[0].channels[i][0];
			myDmx[channels[i]] = inputs->CHOPInputs[1].channels[i][0];	
		}
...
FTDI_ClosePort();
	...
	{
		//output->channels[0][0] = 30;
		//output->channels[1][0] = 20;
		...
		for (int i = 2; i < output->numChannels; i++)
		{
...
void
CPlusPlusCHOPExample::getInfoCHOPChan(int index,
										CHOP_InfoCHOPChan *chan)
{
	...
	if (index == 0)
	{
		chan->name = "Num_Devices";
		chan->value = FTDI_ListDevices();
	}
}

bool		
CPlusPlusCHOPExample::getInfoDATSize(CHOP_InfoDATSize *infoSize)
{
	infoSize->rows = 1;
	infoSize->cols = 2;
...
void
CPlusPlusCHOPExample::getInfoDATEntries(int index,
										int nEntries,
